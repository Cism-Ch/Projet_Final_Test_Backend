...
export const getOwnerTasks = createAsyncThunk('task/getOwnerTasks', async (ownerid) => {
    const {data} = await axios.get(`${URL}/api/v1/tasks/owner/${ownerid}`, {headers: {Authorization: getToken()}});
    return data
})


export const getTasks = createAsyncThunk('task/getTasks', async () => {
    const {data} = await axios.get(`${URL}/api/v1/tasks`, {headers: {Authorization: getToken()}});
    return data
})
